...
//            checking if this communication is possible between the two users
            ...
            ResultSet rs = st_1.executeQuery("select * from item where item_id='" + item_id + "' ");
            if (!rs.next()) {
                ...
                int i = st.executeUpdate("insert into messages (item_id, sender_uname, receiver_uname, time, content) values (" + item_id + ", '" + sender_uname + "', '" + seller_id + "', '" + time + "', '" + content + "')");
                response.sendRedirect("inbox.jsp");
            }
            con.close();
